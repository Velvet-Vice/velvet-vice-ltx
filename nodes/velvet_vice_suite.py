...
def _register_routes() -> None:
    ...
    try:
        ...
        @routes.post("/velvet_vice/ltx/preflight")
        async def velvet_vice_preflight(request):
            try:
                data = await request.json()
            except Exception:
                data = {}
            return web.json_response(run_preflight(data))

        @routes.get("/velvet_vice/ltx/lora/catalog")
        async def velvet_vice_lora_catalog(request):
            return web.json_response(get_lora_catalog())

        @routes.post("/velvet_vice/ltx/lora/analyze")
...
class VelvetVicePowerLoraAV:
    """Power-LoRA-style loader with LTX audio/video routing.
    ...
    audio-to-video attention to reduce direct visual influence.
    """
    ...
    DESCRIPTION = (
        ...
        "LTX audio-video LoRAs. AUDIO excludes audio-to-video LoRA patches."
    )
...
class VelvetViceOutputStudio(VelvetViceLTXAutoVideoCombineV0112):
    CATEGORY = "VELVET VICE/LTX"
    DESCRIPTION = (
        "Velvet Vice Output Studio with authoritative tensor-ratio metadata, "
        "single NVENC/CPU encode, live telemetry and a fully adaptive player frame."
    )
