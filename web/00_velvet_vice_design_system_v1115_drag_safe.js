...
const STYLE_ID = "vv-ltx-design-system-v1115";
...
function marked(node) {
    ...
    return Boolean(props[MARKER] || props.vv_design || props.vv_role || props.vv_badge);
}
...
function drawBody(ctx, node, width, height, titleHeight) {
    ...
    grad.addColorStop(0, "rgba(31,43,54,.98)");
    grad.addColorStop(.55, "rgba(24,33,42,.98)");
    grad.addColorStop(1, "rgba(18,26,34,.98)");
    ctx.fillStyle = grad;
    ...
    sheen.addColorStop(0, "rgba(197,179,215,.12)");
    sheen.addColorStop(.5, "rgba(111,136,159,.08)");
    sheen.addColorStop(1, "rgba(111,136,159,0)");
    ctx.fillStyle = sheen;
    ...
    if (!decorative(node)) {
        for (const item of node.widgets ?? []) {
            ...
            ctx.fillStyle = "rgba(36,48,59,.28)";
            ctx.strokeStyle = "rgba(196,181,214,.055)";
            ctx.lineWidth = .7;
...
function drawHeader(ctx, node, width, titleHeight) {
    ...
    grad.addColorStop(0, active ? "rgba(129,105,153,.98)" : "rgba(109,95,130,.98)");
    grad.addColorStop(.62, active ? "rgba(88,111,132,.95)" : "rgba(83,103,121,.92)");
    grad.addColorStop(1, "rgba(42,54,66,.96)");
    ctx.fillStyle = grad;
    ...
    accent.addColorStop(0, error ? "rgba(212,112,128,.9)" : "rgba(205,186,224,.82)");
    accent.addColorStop(.56, active ? "rgba(132,171,199,.88)" : "rgba(111,137,159,.7)");
    accent.addColorStop(1, "rgba(111,137,159,0)");
    ...
    const dotColor = error ? PALETTE.fail : active ? PALETTE.accent : state === "done" ? PALETTE.ok : "#87929d";
...
function applyTheme(node) {
    ...
    node.onDrawForeground = function(ctx) {
        ...
        ctx.strokeStyle = state === "error" ? "rgba(210,113,128,.75)" : state === "active" ? "rgba(199,180,219,.78)" : PALETTE.border;
        if (state === "active") {
            ctx.shadowColor = "rgba(174,151,199,.46)";
            ctx.shadowBlur = 12;
        }
        ...
        if (!chromeExcluded(this)) {
            ctx.fillStyle = state === "active" ? "rgba(194,174,215,.58)" : "rgba(185,167,207,.34)";
...
window.VelvetViceLTXDesign = Object.freeze({
    version: VERSION,
    palette: PALETTE,
    installCss,
    applyTheme,
...
app.registerExtension({
    name: "VelvetVice.LTX.FullSignatureDesignSystemV1115DragSafe",
    setup() {
        installCss();
