...
const OUTPUT_TYPE = "VelvetViceOutputStudio";
const LORA_TYPE = "VelvetVicePowerLoraAV";
const LEGACY_LORA_TYPE = "VelvetViceLoraStudio";
...
const PROMPT_DIRECTOR_TYPE = "VelvetViceLTXPromptDirector";
const FINAL_PROMPT_TYPE = "VelvetViceLTXFinalPromptPreview";
const WATERMARK_TYPE = "VelvetViceWatermarkOverlay";
const PACK_VERSION = "1.2.7";
...
const PANEL_STYLE_ID = "vv-ltx-panel-suite-v1115";
const OUTPUT_STUDIO_VERSION = "1.1.15-draggable-single-header";
...
function addAdaptiveDOM(node, name, label, element, minHeight, minWidth, chrome = 82) {
    ...
    const domWidget = node.addDOMWidget(name, label, element, {
        ...
        getMaxHeight: () => Math.max(minHeight, measuredHeight),
    ...
    domWidget.computeLayoutSize = () => ({
        ...
        maxHeight: Math.max(minHeight, measuredHeight),
    });
    const resize = () => {
        ...
        if (Math.abs(Number(node.size?.[1] || 0) - targetHeight) > 3) node.setSize([width, targetHeight]);
        node.graph?.setDirtyCanvas?.(true, true);
        node.setDirtyCanvas?.(true, true);
    };
...
function syncProjectNameFromHub(hubNode = null) {
    ...
    const raw = String(value(hub, "project_name", "VELVET_VICE_LTX23_FINAL")).trim();
    const prefix = raw.toLowerCase().startsWith("video/") ? raw : `video/${raw || "VELVET_VICE_LTX23_FINAL"}`;
...
function installControlHub(node) {
    ...
    const { head, badge } = createHeader("VELVET VICE · CONTROL HUB", String(value(node, "edition", "ANTIGHOST")), node);
    ...
    project.value = String(value(node, "project_name", "VELVET_VICE_LTX23_FINAL"));
    ...
    node.setSize([Math.max(node.size?.[0] ?? 580, 580), 500]);
}
...
function graphOllamaUrl() {
    const director = app.graph?._nodes?.find((node) => nodeType(node) === "VelvetViceLTXPromptDirector");
...
function graphPromptMode() {
    const director = app.graph?._nodes?.find((node) => nodeType(node) === PROMPT_DIRECTOR_TYPE);
...
function graphLtxVersion() {
    const node = app.graph?._nodes?.find((item) => nodeType(item) === MODEL_LOADOUT_TYPE);
    const native = nativePanelWidgets(node);
    const values = native.map((item) => String(item?.value ?? "").toLowerCase()).join(" ");
    const title = String(node?.title ?? "").toLowerCase();
    return /ltx[- _]?2[._-]?5|ltx25|gemma4|v1\.5_bf16/.test(`${title} ${values}`) ? "2.5" : "2.3";
}
...
function graphSelectedModels() {
    ...
    const ltxVersion = graphLtxVersion();
    ...
    add(ltxVersion === "2.5" ? "Text encoder + projection" : "Text encoder", 7, ["text_encoders", "clip"], true);
    if (ggufEnabled) add("GGUF text encoder", 8, ["text_encoders", "clip"], true);
    if (ltxVersion === "2.5") add("Prompt enhancer", 17, ["text_encoders", "clip"], true);
    if (Boolean(native[10]?.value ?? false)) add("Distilled LoRA", 11, ["loras"], true);
    return entries;
}

function graphActiveLoras() {
    const node = app.graph?._nodes?.find((item) => nodeType(item) === LORA_TYPE || nodeType(item) === LEGACY_LORA_TYPE);
...
function installPreflight(node) {
    ...
    async function runCheck() {
        ...
        try {
            const response = await api.fetchApi("/velvet_vice/ltx/preflight", {
                method: "POST",
    ...
    node.setSize([Math.max(node.size?.[0] ?? 580, 580), 620]);
...
const STAGES = ["DIRECT", "PREP", "PASS 1", "PASS 2", "PASS 3", "FINISH", "OUTPUT"];

function stageForNode(rawId) {
    const id = String(rawId ?? "");
    const top = id.split(":")[0];
    const leaf = id.split(":").at(-1);
    if (["4035", "4036", "4038", "4037", "3678"].includes(top)) return 0;
    if (top === "3577") return 1;
    if (leaf === "3650") return 2;
    if (leaf === "3649") return 3;
    if (leaf === "3656") return 4;
    if (["3676", "3420", "4000", "4030", "4031", "4040", "4041", "3731", "3747", "3748", "4045"].includes(top)) return 5;
    if (["2196", "4039", "4042"].includes(top)) return 6;
    const graphNode = app.graph?.getNodeById?.(Number(top));
    if (nodeType(graphNode) === OUTPUT_TYPE) return 6;
    return null;
}
...
function installGlobalListeners() {
    ...
    api.addEventListener("executing", ({ detail }) => {
        ...
        if (nodeId == null) {
            updateOutputs("FINALIZING", 6, null, "Waiting for final output");
            return;
        }
    ...
    api.addEventListener("execution_success", () => updateOutputs("COMPLETE", 6, 1, "Final MP4 ready"));
    api.addEventListener("velvet_vice.output_status", ({ detail }) => {
        const stage = String(detail?.stage ?? "OUTPUT");
        updateOutputs(stage, 6, stage === "COMPLETE" ? 1 : null, String(detail?.message ?? ""));
        if (detail?.preview) {
...
function installOutputStudio(node) {
    ...
    const { head, badge } = createHeader("VELVET VICE · OUTPUT STUDIO", "READY", node);
    shell.appendChild(head);
    const body = document.createElement("div");
    body.className = "vv-body";
    shell.appendChild(body);
    ...
    statusDetail.textContent = "Queue the workflow to start the live monitor.";
    statusTextWrap.append(statusTitle, statusDetail);
    const fpsBadge = document.createElement("div");
    fpsBadge.className = "vv-badge";
    fpsBadge.textContent = "SINGLE ENCODE";
    status.append(statusTextWrap, fpsBadge);
    body.appendChild(status);

    const stageRow = document.createElement("div");
    stageRow.className = "vv-stage-row";
    const stageBars = STAGES.map(() => {
        const bar = document.createElement("div");
        bar.className = "vv-stage";
    ...
    stageLabels.style.display = "grid";
    stageLabels.style.gridTemplateColumns = "repeat(7,1fr)";
    stageLabels.style.gap = "5px";
    stageLabels.style.fontSize = "8px";
    stageLabels.style.color = "#77828d";
    stageLabels.style.textAlign = "center";
    for (const label of STAGES) {
        const item = document.createElement("span");
        item.textContent = label;
        stageLabels.appendChild(item);
    }
    body.appendChild(stageLabels);
    ...
    const empty = document.createElement("div");
    empty.className = "vv-empty";
    empty.textContent = "The final MP4 appears here after the single final encode.";
    ...
    const setOutputWidgetHeight = (height) => {
        ...
        domWidget.computeLayoutSize = () => ({
            ...
            maxHeight: outputLayout.height,
        });
        domWidget.options ??= {};
        domWidget.options.getMinHeight = () => outputLayout.height;
        domWidget.options.getMaxHeight = () => outputLayout.height;
    ...
    const forceNodeSize = (width, height) => {
        const next = [Math.round(width), Math.round(height)];
        node.setSize?.(next);
        if (node.size) { node.size[0] = next[0]; node.size[1] = next[1]; }
    ...
    const applyPreviewGeometry = (metaOverride = null) => {
        ...
        requestAnimationFrame(() => {
            ...
            forceNodeSize(layout.nodeWidth, targetHeight);
            requestAnimationFrame(() => forceNodeSize(layout.nodeWidth, targetHeight));
        });
    };
    let resizeQueued = false;
    const queuePreviewGeometry = () => {
        if (resizeQueued || !domWidget.previewMeta?.width || !domWidget.previewMeta?.height) return;
        resizeQueued = true;
        requestAnimationFrame(() => {
            resizeQueued = false;
    ...
    const previewResizeObserver = typeof ResizeObserver !== "undefined"
        ? new ResizeObserver(queuePreviewGeometry)
        : null;
    previewResizeObserver?.observe(shell);
    node.__vvPreviewResizeObserver = previewResizeObserver;
    ...
    node.vvUpdateOutput = (text, index, pct, detailText) => {
        ...
        stageBars.forEach((bar, barIndex) => {
            bar.classList.toggle("done", index != null && barIndex < index);
            bar.classList.toggle("active", index != null && barIndex === index);
            bar.classList.toggle("error", text === "ERROR" && barIndex === (index ?? 0));
        });
        if (pct == null) {
    ...
    node.setSize([Math.max(node.size?.[0] ?? 630, 630), 700]);
}
...
function installModelLoadoutPanel(node) {
    ...
    waitForNativeWidgets(node, 8, (native) => {
        ...
        const ltxVersion = graphLtxVersion();
        const { head, badge } = createHeader("VELVET VICE · MODEL LOADOUT", `LTX ${ltxVersion} · PACK ${PACK_VERSION}`, node); shell.appendChild(head);
        ...
        function activate(index) {
            ...
            badge.textContent = `${groups[index]?.title ?? "LOADOUT"} · LTX ${ltxVersion}`;
            node.setDirtyCanvas?.(true, true);
        }
        ...
        node.setSize([Math.max(node.size?.[0]??660,660),650]);
        activate(0);
    });
}
function installRenderEnginePanel(node) {
    ...
    waitForNativeWidgets(node, 2, (native) => {
        ...
        for (const item of native) hideWidget(item);
        addAdaptiveDOM(node,"vv_render_engine_surface","VELVET VICE RENDER ENGINE",shell,430,700,86); node.setSize([Math.max(node.size?.[0] ?? 700,700),Math.max(node.size?.[1] ?? 540,540)]);
...
function installLoraStudio(node) {
    ...
    const refreshCatalog = async (force = false) => {
        ...
        catalogRequest = (async () => {
            try {
                const response = await api.fetchApi("/velvet_vice/ltx/lora/catalog", { method: "GET" });
                if (!response.ok) throw new Error(`HTTP ${response.status}`);
    ...
    const { head, badge } = createHeader("VELVET VICE · POWER LoRA AV", "0 ACTIVE", node);
    shell.appendChild(head);
    const body = document.createElement("div"); body.className = "vv-body"; shell.appendChild(body);
    ...
    async function analyzeSlot(slot) {
        ...
        try {
            const response = await api.fetchApi("/velvet_vice/ltx/lora/analyze", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({filename:slot.lora})});
    ...
    node.setSize([Math.max(node.size?.[0] ?? 750,750),585]);
    render();
...
function installWatermarkOverlay(node) {
    ...
    node.setSize([Math.max(node.size?.[0] ?? 560, 560), 290]);
    refresh();
}
...
function installVisiblePanel(node) {
    ...
    const type = nodeType(node);
    if ([HUB_TYPE, PREFLIGHT_TYPE, OUTPUT_TYPE, LORA_TYPE, LEGACY_LORA_TYPE].includes(type)) {
        node.__vvSuppressCanvasChromeV1115 = true;
    }
    if (type === HUB_TYPE) installControlHub(node);
    else if (type === PREFLIGHT_TYPE) installPreflight(node);
    else if (type === OUTPUT_TYPE) installOutputStudio(node);
    else if (type === LORA_TYPE || type === LEGACY_LORA_TYPE) installLoraStudio(node);
...
function reassertVisiblePanels() {
    ...
    app.registerExtension({
        name: "VelvetVice.LTX.FullPanelSystemV1115DraggableSingleHeader",
